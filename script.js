...
//                            <h6>Subject: ${record.subject}</h6>
//                            <h6>Subject: ${record.sem}</h6>
//                            <h6>Subject: ${record.class}</h6>
...
function searchForRecord(data, enroll, grno) {
  const subject = data[0][1];   // Row 0, column B
  const sem = data[1][1];       // Row 1, column B
  const className = data[2][1]; // Row 2, column B

  for (let i = 4; i < data.length; i++) { // Skip 0-3 (metadata + headers)
    const row = data[i];
    if (row[0] == enroll && row[1] == grno) {
      return {
        name: row[2],
...
function displayResult(record) {
  ...
  if (record) {
    resultDiv.innerHTML = `<h5>Result Found!</h5><br>
                           <h6>Name: ${record.name}</h6>
                           <h6>Subject: ${record.subject}</h6>
                           <h6>Semester: ${record.sem}</h6>
                           <h6>Class: ${record.class}</h6>
                           <h6>Marks: ${record.marks}</h6>`;
  } else {
    resultDiv.innerHTML = `<p class="text-danger">No matching record found!!</p>`;
  }
